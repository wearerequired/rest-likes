...
use Required\RestLikes\ObjectType\ObjectType;
use WP_Error;
use WP_REST_Server;
...
class Controller extends WP_REST_Controller {
	 ...
	 *
	 * @param ObjectType $object_type Object type this controller is for.
	 */
	public function __construct( ObjectType $object_type  ) {
	...
	public function add_hooks() {
		...
		$this->object_type->add_hooks();
	}

	public function get_script_data() {
		return [
			'endpoint'   => $this->get_namespace() . $this->get_rest_route_placeholder(),
			'classnames' => $this->get_classnames(),
			'texts'      => [
				/** This filter is documented in classes/Controller.php */
				'like'   => apply_filters( 'rest_likes.button_text.like', _x( 'Like', 'verb', 'rest-likes' ), $this->get_object_type() ),
				/**
				 ...
				 */
				'unlike' => apply_filters( 'rest_likes.button_text.unlike', _x( 'Unlike', 'verb', 'rest-likes' ), $this->get_object_type() ),
			],
		];
	}
	...
	protected function get_rest_field_object_type() {
		return $this->object_type->get_rest_field_object_type();
	}
	...
	public function check_permission( WP_REST_Request $request ) {
		...
		return $this->object_type->check_permission( $request );
	}
	...
	public function add_like( $request ) {
		...
		return new WP_REST_Response( $this->handle_like( $request['id'] ), 201 );
	}
	...
	public function get_like_button( $object_id ) {
		if ( ! $this->object_type->is_allowed_object_id( $object_id ) ) {
			return '';
		}
	...
	public function get_item_schema() {
		$schema = [
			...
			'title'      => $this->object_type->get_name(),
			'type'       => 'object',
			'properties' => [
				'count'          => [
